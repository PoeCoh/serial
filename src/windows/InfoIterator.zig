...
fn getPortName(device_info_set: *const HDEVINFO, device_info_data: *SP_DEVINFO_DATA, port_name: [*]u8) std.os.windows.DWORD {
    const hkey: std.os.windows.HKEY = SetupDiOpenDevRegKey(
        device_info_set.*,
        device_info_data,
...
const Information = @import("../Information.zig");
...
extern "setupapi" fn SetupDiGetClassDevsW(
    ...
    hwndParanet: ?std.os.windows.HWND,
...
) callconv(std.os.windows.WINAPI) std.os.windows.HKEY;
